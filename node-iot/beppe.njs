...
function parseAnna(msg) {
	var clearData = atob(msg.message.data);
	console.log("parseAnna: " + clearData);
...
function parseAdmin(msg) {
	var clearData = atob(msg.message.data);
	console.log("parseAdmin: " + clearData);
	switch (clearData) {
	case "Pronti?":
...
jwtClient.authorize(function(err, tokens) {
	if (err === null) {
		...
		pullLoop(ANNA_SUB, parseAnna);

		pullLoop(ADMIN_SUB, parseAdmin);
	} else {
		console.log(err);
	}
});
